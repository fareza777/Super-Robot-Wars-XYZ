...
const UnitCell = React.memo(function UnitCell({ u, chip, ghosting, threat }: { u: UnitState; chip: number; ghosting: boolean; threat?: boolean }) {
  ...
  return (
    <Animated.View style={{ alignItems: 'center', transform: [{ translateY: bob }] }}>
      ...
      {(() => { const d = (u.doomTurns ? 1 : 0) + (u.sirenTurns ? 1 : 0) + (u.obscuredTurns ? 1 : 0) + (u.rustTurns ? 1 : 0) + (u.stifleTurns ? 1 : 0) + (u.rotTurns ? 1 : 0) + (u.frailTurns ? 1 : 0) + (u.gloomTurns ? 1 : 0) + (u.poxTurns ? 1 : 0) + (u.glitchUntilEndOfEnemyPhase ? 1 : 0) + (u.mapSealUntilEndOfEnemyPhase ? 1 : 0) + (u.resistSealUntilEndOfEnemyPhase ? 1 : 0) + (u.baneUntilEndOfEnemyPhase ? 1 : 0) + (u.cursedUntilEndOfEnemyPhase ? 1 : 0) + (u.weakenUntilEndOfEnemyPhase ? 1 : 0) + (u.silencedUntilEndOfEnemyPhase ? 1 : 0) + (u.nullifiedUntilEndOfEnemyPhase ? 1 : 0) + (u.tetherUntilEndOfEnemyPhase ? 1 : 0) + (u.exposed ? 1 : 0) + (u.rended ? 1 : 0) + (u.sundered ? 1 : 0) + (u.crippled ? 1 : 0); return ((u.statuses?.length ?? 0) + d) > 0 ? <Text style={styles.statusTag}>{(u.statuses ?? []).map((fx) => (fx.id === 'burn' ? '🔥' : fx.id === 'stun' ? '⚡' : fx.id === 'slow' ? '🕸' : fx.id === 'mark' ? '🎯' : fx.id === 'supp' ? '🔻' : '⬇')).join('')}{d > 0 ? `⌛${d}` : ''}</Text> : null; })()}
      {u.will > 100 && (
