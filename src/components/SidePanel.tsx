      ...
      <View style={{ flexDirection: 'row', alignItems: 'center' }}>
        <Pressable onPress={() => setShowRoster((v) => !v)} style={({ pressed }) => [{ flex: 1 }, pressed && { opacity: 0.7 }]}>
          <Text style={styles.counts}>
            <Text style={{ color: '#8affc0' }}>Ally {alivePlayers(s).length}</Text> · <Text style={{ color: '#ff8a8a' }}>Enemy</Text> {s.missionCh.fog ? `${aliveEnemies(s).filter((e) => fogLit(s.units, e.pos)).length}/${aliveEnemies(s).length}` : aliveEnemies(s).length} · <Text style={{ color: '#ffd34d' }}>☠{s.kills}</Text>{s.chainCount >= 2 ? <Text style={{ color: '#ffb84d' }}> ⛓{s.chainCount}</Text> : null}{s.crates.length > 0 ? <Text style={{ color: '#ffb84d' }}> · ▣{s.crates.length}</Text> : null}  · <Text style={{ color: '#9fd8ff' }}>T{s.turn}</Text> · <Text style={{ color: '#ffd34d' }}>¢{s.credits >= 1000 ? `${Math.floor(s.credits / 1000)}k` : s.credits}</Text> · <Text style={{ color: '#c9a0ff' }}>✦{s.units.filter((x) => x.alive && x.side === 'player').reduce((a, x) => a + x.sp, 0)}</Text> · <Text style={{ color: '#8af0ff' }}>⛽{Math.round(s.units.filter((x) => x.alive && x.side === 'player').reduce((a, x) => a + x.en, 0) / Math.max(1, s.units.filter((x) => x.alive && x.side === 'player').length))}</Text> {(() => { const lost = s.units.filter((u) => u.side === 'player' && !u.alive).length; const par = s.missionCh.surviveTurns ?? Math.max(6, Math.ceil(((s.missionCh.count ?? 0) + (s.missionCh.boss ? 1 : 0)) * 1.1)); const sc = Math.max(0, Math.min(100, 100 - lost * 20 - Math.max(0, s.turn - 1 - par) * 6)); const r = sc >= 95 ? 'S' : sc >= 80 ? 'A' : sc >= 60 ? 'B' : 'C'; return <Text style={{ color: r === 'S' ? '#ffd34d' : r === 'A' ? '#8affc0' : r === 'B' ? '#8af0ff' : '#ff8a8a' }}> · ≈{r}</Text>; })()}{showRoster ? '▲' : '▼'}
          </Text>
        </Pressable>
