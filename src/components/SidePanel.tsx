...
function buffNames(u: UnitState): string[] {
  ...
  if ((u.poxTurns ?? 0) > 0) names.push(`POX VERSE ${u.poxTurns}`);
  ...
  if (u.markbrandUntilEndOfEnemyPhase) names.push('HUNT VERSE');
  if (u.breakbrandUntilEndOfEnemyPhase) names.push('REND VERSE');
  if (u.slowbrandUntilEndOfEnemyPhase) names.push('RIME VERSE');
  if (u.suppbrandUntilEndOfEnemyPhase) names.push('STATIC VERSE');
  if (u.stunbrandUntilEndOfEnemyPhase) names.push('PEAL VERSE');
  if (u.piercebrandUntilEndOfEnemyPhase) names.push('PIERCE VERSE');
  if (u.banebrandUntilEndOfEnemyPhase) names.push('CARNAGE VERSE');
  ...
  if (u.huskedgeNext) names.push('HUSK EDGE');
  if (u.festeredgeNext) names.push('FESTER EDGE');
  if (u.drainedgeNext) names.push('DRAIN EDGE');
  if (u.bulwarkedgeNext) names.push('BULWARK EDGE');
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center' }}>
        <Pressable onPress={() => setShowRoster((v) => !v)} style={({ pressed }) => [{ flex: 1 }, pressed && { opacity: 0.7 }]}>
          <Text style={styles.counts}>
            <Text style={{ color: '#8affc0' }}>Ally {alivePlayers(s).length}</Text> · <Text style={{ color: '#ff8a8a' }}>Enemy</Text> {s.missionCh.fog ? `${aliveEnemies(s).filter((e) => fogLit(s.units, e.pos)).length}/${aliveEnemies(s).length}` : aliveEnemies(s).length} · <Text style={{ color: '#ffd34d' }}>☠{s.kills}</Text>{s.chainCount >= 2 ? <Text style={{ color: '#ffb84d' }}> ⛓{s.chainCount}</Text> : null}{s.crates.length > 0 ? <Text style={{ color: '#ffb84d' }}> · ▣{s.crates.length}</Text> : null}{s.units.filter((x) => x.side === 'enemy' && x.alive && debuffCount(x) > 0).length > 0 ? <Text style={{ color: '#ff9d7a' }}> · ⌛{s.units.filter((x) => x.side === 'enemy' && x.alive && debuffCount(x) > 0).length}</Text> : null}{s.units.filter((x) => x.side === 'enemy' && x.alive && x.elite).length > 0 ? <Text style={{ color: '#ffd34d' }}> · ★{s.units.filter((x) => x.side === 'enemy' && x.alive && x.elite).length}</Text> : null}{s.units.filter((x) => x.side === 'enemy' && x.alive && Math.max(x.def.barrier ?? 0, partBonus(x, 'barrier'), x.wallUntilEndOfEnemyPhase ? 500 : 0, x.haloUntilEndOfEnemyPhase ? 400 : 0) > 0).length > 0 ? <Text style={{ color: '#8af0ff' }}> · ⛨{s.units.filter((x) => x.side === 'enemy' && x.alive && Math.max(x.def.barrier ?? 0, partBonus(x, 'barrier'), x.wallUntilEndOfEnemyPhase ? 500 : 0, x.haloUntilEndOfEnemyPhase ? 400 : 0) > 0).length}</Text> : null}{s.units.filter((x) => x.side === 'enemy' && x.alive && x.def.weapons.some((w) => w.mapRange)).length > 0 ? <Text style={{ color: '#ff9d7a' }}> · ◉{s.units.filter((x) => x.side === 'enemy' && x.alive && x.def.weapons.some((w) => w.mapRange)).length}</Text> : null}{s.units.filter((x) => x.side === 'enemy' && x.alive && x.def.weapons.some((w) => w.sniper)).length > 0 ? <Text style={{ color: '#9fd8ff' }}> · ⌖{s.units.filter((x) => x.side === 'enemy' && x.alive && x.def.weapons.some((w) => w.sniper)).length}</Text> : null}{s.units.filter((x) => x.side === 'enemy' && x.alive && x.def.moveType === 'air').length > 0 ? <Text style={{ color: '#8af0ff' }}> · ✈{s.units.filter((x) => x.side === 'enemy' && x.alive && x.def.moveType === 'air').length}</Text> : null}{s.units.filter((x) => x.side === 'enemy' && x.alive && x.provokedTo).length > 0 ? <Text style={{ color: '#ffd34d' }}> · 🎯{s.units.filter((x) => x.side === 'enemy' && x.alive && x.provokedTo).length}</Text> : null}{s.units.filter((x) => x.side === 'enemy' && x.alive && usableWeapons(x).length === 0).length > 0 ? <Text style={{ color: '#ff6a6a' }}> · ⊘{s.units.filter((x) => x.side === 'enemy' && x.alive && usableWeapons(x).length === 0).length}</Text> : null}{s.units.filter((x) => x.side === 'enemy' && x.alive && x.def.medic).length > 0 ? <Text style={{ color: '#8affc0' }}> · ✚{s.units.filter((x) => x.side === 'enemy' && x.alive && x.def.medic).length}</Text> : null}{s.units.filter((x) => x.side === 'enemy' && x.alive && x.def.weapons.some((w) => w.status)).length > 0 ? <Text style={{ color: '#b8ff7a' }}> · ☣{s.units.filter((x) => x.side === 'enemy' && x.alive && x.def.weapons.some((w) => w.status)).length}</Text> : null}{s.units.filter((x) => x.side === 'player' && x.alive && (x.will ?? 0) >= 120).length > 0 ? <Text style={{ color: '#8af0ff' }}> · Ⓦ{s.units.filter((x) => x.side === 'player' && x.alive && (x.will ?? 0) >= 120).length}</Text> : null}{s.units.filter((x) => x.side === 'player' && !x.alive).length > 0 ? <Text style={{ color: '#ff6a6a' }}> · ✖{s.units.filter((x) => x.side === 'player' && !x.alive).length}</Text> : null}{s.units.filter((x) => x.side === 'player' && x.alive && x.againOnKill).length > 0 ? <Text style={{ color: '#ffd34d' }}> · ⚡{s.units.filter((x) => x.side === 'player' && x.alive && x.againOnKill).length}</Text> : null}{s.units.filter((x) => x.side === 'enemy' && x.alive && x.def.boss).length > 0 ? <Text style={{ color: '#ff8a8a' }}> · ♚{s.units.filter((x) => x.side === 'enemy' && x.alive && x.def.boss).length}</Text> : null}{s.units.filter((x) => x.side === 'enemy' && x.alive && x.silencedUntilEndOfEnemyPhase).length > 0 ? <Text style={{ color: '#9fd8ff' }}> · ⊗{s.units.filter((x) => x.side === 'enemy' && x.alive && x.silencedUntilEndOfEnemyPhase).length}</Text> : null}{s.units.filter((x) => x.side === 'player' && x.alive && x.hp <= x.def.maxHp * 0.5).length > 0 ? <Text style={{ color: '#ff9d4d' }}> · ◐{s.units.filter((x) => x.side === 'player' && x.alive && x.hp <= x.def.maxHp * 0.5).length}</Text> : null}{s.units.filter((x) => x.side === 'enemy' && x.alive && x.def.carrier).length > 0 ? <Text style={{ color: '#ffe8a0' }}> · 💰{s.units.filter((x) => x.side === 'enemy' && x.alive && x.def.carrier).length}</Text> : null}{s.units.filter((x) => x.side === 'player' && x.alive && Math.max(x.def.barrier ?? 0, partBonus(x, 'barrier'), x.wallUntilEndOfEnemyPhase ? 500 : 0, x.haloUntilEndOfEnemyPhase ? 400 : 0) > 0).length > 0 ? <Text style={{ color: '#8af0ff' }}> · ⊞{s.units.filter((x) => x.side === 'player' && x.alive && Math.max(x.def.barrier ?? 0, partBonus(x, 'barrier'), x.wallUntilEndOfEnemyPhase ? 500 : 0, x.haloUntilEndOfEnemyPhase ? 400 : 0) > 0).length}</Text> : null}{s.units.filter((x) => x.side === 'player' && x.alive && x.luckyForNextKill).length > 0 ? <Text style={{ color: '#ffe8a0' }}> · ✦{s.units.filter((x) => x.side === 'player' && x.alive && x.luckyForNextKill).length}</Text> : null}{s.units.filter((x) => x.side === 'enemy' && x.alive && x.hp <= x.def.maxHp * 0.5).length > 0 ? <Text style={{ color: '#ff8a8a' }}> · ◑{s.units.filter((x) => x.side === 'enemy' && x.alive && x.hp <= x.def.maxHp * 0.5).length}</Text> : null}  · <Text style={{ color: '#9fd8ff' }}>T{s.turn}</Text> · <Text style={{ color: '#ffd34d' }}>¢{s.credits >= 1000 ? `${Math.floor(s.credits / 1000)}k` : s.credits}</Text> · <Text style={{ color: '#c9a0ff' }}>✦{s.units.filter((x) => x.alive && x.side === 'player').reduce((a, x) => a + x.sp, 0)}</Text> · <Text style={{ color: '#8af0ff' }}>⛽{Math.round(s.units.filter((x) => x.alive && x.side === 'player').reduce((a, x) => a + x.en, 0) / Math.max(1, s.units.filter((x) => x.alive && x.side === 'player').length))}</Text> {(() => { const lost = s.units.filter((u) => u.side === 'player' && !u.alive).length; const par = s.missionCh.surviveTurns ?? Math.max(6, Math.ceil(((s.missionCh.count ?? 0) + (s.missionCh.boss ? 1 : 0)) * 1.1)); const sc = Math.max(0, Math.min(100, 100 - lost * 20 - Math.max(0, s.turn - 1 - par) * 6)); const r = sc >= 95 ? 'S' : sc >= 80 ? 'A' : sc >= 60 ? 'B' : 'C'; return <Text style={{ color: r === 'S' ? '#ffd34d' : r === 'A' ? '#8affc0' : r === 'B' ? '#8af0ff' : '#ff8a8a' }}> · ≈{r}</Text>; })()}{showRoster ? '▲' : '▼'}
          </Text>
        </Pressable>
          ...
          <View style={styles.menu}>
            ...
            {unit.def.weapons.map((w) => {
              ...
              return (
                <Btn
                  key={w.id}
                  label={`${w.kind === 'gun' ? '⌖' : w.kind === 'beam' ? '✦' : w.kind === 'missile' ? '▲' : w.kind === 'funnel' ? '◈' : '⚔'} ${w.name}${w.mapRange != null ? ' ◉' : ''}${w.status ? ' ☣' : ''}${w.willReq ? ` W${w.willReq}` : ''}${(w.multiHit ?? 1) > 1 ? ` ×${w.multiHit}` : ''}${reason ? ` · ${reason}` : ''}`}
                  sub={stat}
          ...
          <View style={styles.rosterBox}>
            ...
            <ScrollView style={{ maxHeight: 92 }} nestedScrollEnabled>
              {s.units
                ...
                .sort((a, b) => a.hp / a.def.maxHp - b.hp / b.def.maxHp)
                .map((u) =>
                  s.missionCh.fog && !fogLit(s.units, u.pos) ? (
                    ...
                    <Pressable key={u.uid} style={styles.rosterRow} onPress={() => useGame.setState({ inspectUid: u.uid })}>
                      <Text style={[styles.rosterName, { color: '#ffb0a0' }]} numberOfLines={1}>
                        ...
                        {debuffCount(u) > 0 ? <Text style={{ color: '#ff9d7a' }}> ⌛{debuffCount(u)}</Text> : null}{u.def.weapons.some((w) => w.kind === 'beam') ? <Text style={{ color: '#8af0ff' }}> ⌁</Text> : null}{u.def.weapons.some((w) => w.status) ? <Text style={{ color: '#b8ff7a' }}> ☣</Text> : null}
                        {buffNames(u).length > 0 ? <Text style={{ color: '#d8a5ff' }}> ✧{buffNames(u).length}</Text> : null}
                        {u.crippled ? <Text style={{ color: '#ff9d7a' }}> ⛓</Text> : null}
                        {u.def.weapons.some((w) => w.mapRange != null) ? <Text style={{ color: '#ff9d4d' }}> ◉</Text> : null}{u.def.weapons.some((w) => w.sniper) ? <Text style={{ color: '#9fd8ff' }}> ⌖</Text> : null}{Math.max(u.def.barrier ?? 0, partBonus(u, 'barrier'), u.wallUntilEndOfEnemyPhase ? 500 : 0, u.haloUntilEndOfEnemyPhase ? 400 : 0) > 0 ? <Text style={{ color: '#8af0ff' }}> ⛨</Text> : null}
        ...
        {inspect && !s.menuForUid && !s.pendingWeapon && (
          <View style={[styles.unitCard, styles.inspectCard]}>
            ...
            <View style={styles.statRow}>
              ...
              <Text style={styles.statTxt}>▸ MOV {moveRangeOf(inspect)} {inspect.def.moveType === 'air' ? '✈' : '⬢'}{inspect.repulseUntilEndOfEnemyPhase || partBonus(inspect, 'counterRange') > 0 ? ' ↩+1' : ''}{debuffCount(inspect) > 0 ? ` · ⌛${debuffCount(inspect)}` : ''}</Text>
            </View>
            ...
            {inspect.side === 'enemy' && <Text style={{ color: '#ffd34d', fontSize: 10.5, marginTop: 3 }}>☠ KILL YIELDS +{inspect.elite ? 110 : 70} EXP{inspect.def.boss ? ' · Ω BOSS BOUNTY' : ''}</Text>}
            {inspect.side === 'player' && (inspect.def.repairer || inspect.def.supplier) && <Text style={{ color: '#4dff9d', fontSize: 10.5, marginTop: 3 }}>{inspect.def.repairer ? '✚ REPAIR adjacent ally — +40% HP +30 EN' : ''}{inspect.def.repairer && inspect.def.supplier ? ' · ' : ''}{inspect.def.supplier ? '▣ RESUPPLY ≤3t — +50 EN + ammo' : ''}</Text>}
