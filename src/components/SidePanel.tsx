          ...
          <View style={styles.menu}>
            ...
            {unit.def.weapons.map((w) => {
              ...
              return (
                <Btn
                  key={w.id}
                  label={`${w.kind === 'gun' ? '⌖' : w.kind === 'beam' ? '✦' : w.kind === 'missile' ? '▲' : w.kind === 'funnel' ? '◈' : '⚔'} ${w.name}${w.mapRange != null ? ' ◉' : ''}${w.status ? ' ☣' : ''}${(w.multiHit ?? 1) > 1 ? ` ×${w.multiHit}` : ''}${reason ? ` · ${reason}` : ''}`}
                  sub={stat}
