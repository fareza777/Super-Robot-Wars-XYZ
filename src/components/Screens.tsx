          ...
          {roster.map((id) => {
            ...
            return (
              <Pressable key={id} style={({ pressed }) => [styles.squadCard, !on && { opacity: 0.35 }, on && { borderColor: '#4dff7a', shadowColor: '#4dff7a', shadowOpacity: 0.4, shadowRadius: 6, elevation: 4 }, pressed && { opacity: 0.6, transform: [{ scale: 0.94 }] }]} onPress={() => toggleDeploy(id)}>
                ...
                <Text style={{ color: '#6b7694', fontSize: 8 }} numberOfLines={1}>MOV {d.moveRange} {d.moveType === 'air' ? '✈' : '⬢'} · ✦{d.pilot.maxSp} SP · ARMX{d.weapons.length} · RNG {Math.min(...d.weapons.map((w) => w.rangeMin))}-{Math.max(...d.weapons.map((w) => w.rangeMax))}</Text>
