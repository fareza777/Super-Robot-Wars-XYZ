  ...
  return (
    <View style={styles.center}>
      ...
      <View style={styles.briefBox}>
        ...
        <Text style={styles.deployLbl}>DEPLOY SQUAD — tap to toggle ({deploySel.length}/{roster.length}) · ΣPWR {deploySel.reduce((n, id2) => n + Math.max(0, ...(ALL_UNITS[id2]?.weapons ?? []).map((w) => w.power)), 0)} · ΣHP {deploySel.reduce((n, id2) => n + (ALL_UNITS[id2]?.maxHp ?? 0), 0)}{useGame.getState().crates.length > 0 ? ` · ▣${useGame.getState().crates.length}` : ''}</Text>
          ...
          {roster.map((id) => {
            ...
            return (
              <Pressable key={id} style={({ pressed }) => [styles.squadCard, !on && { opacity: 0.35 }, on && { borderColor: '#4dff7a', shadowColor: '#4dff7a', shadowOpacity: 0.4, shadowRadius: 6, elevation: 4 }, pressed && { opacity: 0.6, transform: [{ scale: 0.94 }] }]} onPress={() => toggleDeploy(id)}>
                ...
                <Text style={{ color: '#6b7694', fontSize: 8 }} numberOfLines={1}>MOV {d.moveRange} {d.moveType === 'air' ? '✈' : '⬢'} · ✦{d.pilot.maxSp} SP · ⛽{d.maxEn} EN · ARMX{d.weapons.length} · ⚡{Math.max(0, ...d.weapons.map((w) => w.power))} · RNG {Math.min(...d.weapons.map((w) => w.rangeMin))}-{Math.max(...d.weapons.map((w) => w.rangeMax))}</Text>
                ...
                <Text style={{ color: '#c9a0ff', fontSize: 8, fontWeight: '700' }}>✦ {d.pilot.spirits.length} SPIRITS</Text>{d.barrier ? <Text style={{ color: '#8af0ff', fontSize: 8, fontWeight: '700' }}> ⛨{d.barrier}</Text> : null}
