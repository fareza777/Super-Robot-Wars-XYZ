...
export function moveRangeOf(u: UnitState): number {
  return Math.max(1, u.def.moveRange + (u.accelThisTurn ?? 0) + partBonus(u, 'move') + (u.tideUntilEndOfEnemyPhase ? 1 : 0) + (u.def.moveType === 'air' ? partBonus(u, 'skyBooster') : 0) - (u.ebbUntilEndOfEnemyPhase ? 1 : 0) - (u.crippled ? 2 : 0) - (u.statuses?.some((fx) => fx.id === 'slow') ? 3 : 0));
}
...
export const rangeMaxOf = (u: UnitState, w: WeaponDef) => w.rangeMax + (u.snipeForNextAttack ? 2 : 0) + (u.firelinkUntilEndOfEnemyPhase ? 1 : 0) + (w.kind === 'melee' ? 0 : partBonus(u, 'range'));
...
export function damageOf(att: UnitState, def: UnitState, w: WeaponDef, map: MapDef, crit: boolean, dmgMult = 1, units?: UnitState[]): number {
  ...
  if (w.knockback) dmg = Math.round(dmg * (1 + (att.skills?.sledge ?? 0) * 0.06));
  ...
  dmg = Math.round(dmg * (1 + (units?.filter((a) => a.alive && a.side === att.side && a.uid !== att.uid && partBonus(a, 'auraDmg') > 0 && dist(a.pos, att.pos) <= 2).length ?? 0) * 0.08));
...
export const critChance = (att: UnitState, def: UnitState, w?: WeaponDef) => def.aegisUntilEndOfEnemyPhase || partBonus(def, 'critGuard') > 0 ? 0 : Math.max(5, Math.round(8 + (att.def.mobility - def.def.mobility) * 0.2 + (w?.critMod ?? 0) + partBonus(att, 'crit') + (att.charged ? 15 : 0) + (att.marksmanUntilEndOfEnemyPhase ? 15 : 0) + (att.enraged ? 10 : 0) + (att.skills?.crit ?? 0) * 2));
...
export function bestCounterWeapon(def: UnitState, attPos: Pos): WeaponDef | undefined {
  const opts = usableWeapons(def).filter((w) => dist(def.pos, attPos) >= w.rangeMin && dist(def.pos, attPos) <= rangeMaxOf(def, w) + partBonus(def, 'counterRange') && !w.mapRange);
...
export function applyAttack(state: GameState, attackerUid: string, defenderUid: string, weaponId: string, mods?: (u: UnitState) => CombatMods, reaction: Reaction = 'counter'): { state: GameState; result: AttackResult } {
  ...
  if (result.hit) {
    ...
    if (result.destroyed && def.standFirmUntilEndOfEnemyPhase) {
      result.destroyed = false;
      result.miracle = true;
  ...
  att.judgeNext = false;
  att.goreNext = false; att.rampageNext = false; att.hemoNext = false; att.voidedgeNext = false; att.plunderNext = false; att.knockNext = false; att.lacerateNext = false; att.hellfireNext = false; att.thrillNext = false; att.sunderstormNext = false; att.ravenousNext = false;
...
export function applySupportStrike(
  ...
  const r = resolveHit(sup, def, w, state.map, { hitBonus: m.hitBonus, dmgMult: m.dmgMult * (0.55 + partBonus(sup, 'supportDmg') / 100) });
...
export function phaseRecovery(u: UnitState, map: MapDef, units?: UnitState[]): { hpGain: number; enGain: number; hpLoss: number } {
  ...
  const enGain = Math.min(u.def.maxEn - u.en, 5 + (t.enRegen ?? 0) + partBonus(u, 'enRegen') + aura + partBonus(u, 'ventEn'));
  const hpGain = Math.min(u.def.maxHp - u.hp, Math.round(u.def.maxHp * (t.hpRegen ?? 0)) + Math.round(u.def.maxHp * (partBonus(u, 'hpRegen') / 100)) + Math.round(u.def.maxHp * 0.01 * (u.skills?.regen ?? 0)) + Math.round(u.def.maxHp * healPct / 100) + (partBonus(u, 'lowHpRegen') > 0 && u.hp < u.def.maxHp * 0.4 ? Math.round(u.def.maxHp * 0.08) : 0) + (u.graceTurns ? Math.round(u.def.maxHp * 0.05) : 0));
  if (u.graceTurns) u.graceTurns -= 1;
  const hpLoss = partBonus(u, 'terraProof') > 0 ? 0 : Math.min(u.hp - 1, Math.round(u.def.maxHp * (t.hpDmg ?? 0))); // terrain can't kill — leaves 1 HP
