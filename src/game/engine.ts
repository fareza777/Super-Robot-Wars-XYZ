...
export function damageOf(att: UnitState, def: UnitState, w: WeaponDef, map: MapDef, crit: boolean, dmgMult = 1, units?: UnitState[]): number {
  ...
  if ((att.skills?.purist ?? 0) > 0 && !(def.statuses?.length) && !def.exposed && !def.rended && !def.sundered && !def.crippled && !(def.doomTurns ?? 0) && !(def.sirenTurns ?? 0) && !def.cursedUntilEndOfEnemyPhase && !def.weakenUntilEndOfEnemyPhase && !def.silencedUntilEndOfEnemyPhase && !(def.rustTurns ?? 0) && !(def.stifleTurns ?? 0) && !(def.obscuredTurns ?? 0) && !(def.rotTurns ?? 0) && !(def.frailTurns ?? 0) && !(def.gloomTurns ?? 0) && !(def.poxTurns ?? 0) && !def.glitchUntilEndOfEnemyPhase && !def.mapSealUntilEndOfEnemyPhase && !def.resistSealUntilEndOfEnemyPhase) dmg = Math.round(dmg * (1 + att.skills.purist * 0.05));
  ...
  if ((att.skills?.plagueborn ?? 0) > 0 && ((def.statuses?.length ?? 0) + (def.doomTurns ? 1 : 0) + (def.sirenTurns ? 1 : 0) + (def.exposed ? 1 : 0) + (def.rended ? 1 : 0) + (def.sundered ? 1 : 0) + (def.crippled ? 1 : 0) + (def.silencedUntilEndOfEnemyPhase ? 1 : 0) + (def.cursedUntilEndOfEnemyPhase ? 1 : 0) + (def.weakenUntilEndOfEnemyPhase ? 1 : 0) + (def.obscuredTurns ? 1 : 0) + (def.nullifiedUntilEndOfEnemyPhase ? 1 : 0) + (def.tetherUntilEndOfEnemyPhase ? 1 : 0) + (def.rustTurns ? 1 : 0) + (def.stifleTurns ? 1 : 0) + (def.rotTurns ? 1 : 0) + (def.frailTurns ? 1 : 0) + (def.gloomTurns ? 1 : 0) + (def.poxTurns ? 1 : 0) + (def.glitchUntilEndOfEnemyPhase ? 1 : 0) + (def.mapSealUntilEndOfEnemyPhase ? 1 : 0) + (def.resistSealUntilEndOfEnemyPhase ? 1 : 0)) >= 3) dmg = Math.round(dmg * (1 + att.skills.plagueborn * 0.04));
