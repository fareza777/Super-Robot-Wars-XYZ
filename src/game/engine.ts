...
export function damageOf(att: UnitState, def: UnitState, w: WeaponDef, map: MapDef, crit: boolean, dmgMult = 1, units?: UnitState[]): number {
  ...
  const barrier = (att.pierceNext || def.sieveUntilEndOfEnemyPhase) ? 0 : Math.max(def.def.barrier ?? 0, partBonus(def, 'barrier'), def.wallUntilEndOfEnemyPhase ? 500 : 0, def.haloUntilEndOfEnemyPhase ? 400 : 0);
