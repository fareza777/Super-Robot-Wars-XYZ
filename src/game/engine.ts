...
export function moveRangeOf(u: UnitState): number {
  return Math.max(1, u.def.moveRange + (u.accelThisTurn ?? 0) + partBonus(u, 'move') + (u.tideUntilEndOfEnemyPhase ? 1 : 0) + (u.swiftUntilEndOfEnemyPhase ? 1 : 0) + (u.hasteUntilEndOfEnemyPhase ? 1 : 0) + (u.def.moveType === 'air' ? partBonus(u, 'skyBooster') : 0) + (u.en > u.def.maxEn * 0.75 ? partBonus(u, 'pulseVernier') : 0) + (u.def.moveType === 'land' ? partBonus(u, 'landVernier') : 0) - (u.ebbUntilEndOfEnemyPhase ? 1 : 0) - (u.tetherUntilEndOfEnemyPhase ? 1 : 0) - (u.rootedUntilEndOfEnemyPhase ? 99 : 0) - (u.crippled ? 2 : 0) - (u.statuses?.some((fx) => fx.id === 'slow') ? 3 : 0));
}
...
export function enCostOf(u: UnitState, w: WeaponDef): number {
  ...
  return Math.max(0, (mult < 1 ? Math.max(1, Math.round(w.enCost * mult)) : w.enCost) + (u.surtaxUntilEndOfEnemyPhase ? 10 : 0) - (u.freeflowUntilEndOfEnemyPhase ? 8 : 0));
}
...
export const willGain = (u: UnitState, n: number) => {
  if (u.huskUntilEndOfEnemyPhase) return;
  u.will = Math.max(100, Math.min(MAX_WILL, u.will + n));
};

/** Weapon's effective max range — Snipe spirit adds +2 for the next attack. */
export const rangeMaxOf = (u: UnitState, w: WeaponDef) => w.rangeMax + (u.snipeForNextAttack ? 2 : 0) + (u.firelinkUntilEndOfEnemyPhase ? 1 : 0) + (w.kind === 'melee' ? 0 : partBonus(u, 'range')) + (u.seraphUntilEndOfEnemyPhase && w.kind !== 'melee' ? 1 : 0) - (u.tangleUntilEndOfEnemyPhase ? 1 : 0);
...
export function damageOf(att: UnitState, def: UnitState, w: WeaponDef, map: MapDef, crit: boolean, dmgMult = 1, units?: UnitState[]): number {
  ...
  if (att.lanceUntilEndOfEnemyPhase) armor = Math.round(armor * 0.8);
  if ((att.skills?.perforator ?? 0) > 0) armor = Math.round(armor * (1 - att.skills.perforator * 0.1));
  ...
  if (crit) dmg = Math.round(dmg * (1.3 + (att.skills?.precision ?? 0) * 0.05 + (att.skills?.vitals ?? 0) * 0.05 + (att.slayUntilEndOfEnemyPhase ? 0.35 : 0)));
  ...
  if (att.pierceNext) dmg = Math.round(dmg * 1.10);
  if (att.muteNext) dmg = Math.round(dmg * 1.10);
  if (att.dragNext) dmg = Math.round(dmg * 1.1);
  if (att.blinkNext) dmg = Math.round(dmg * 1.1);
  if (att.curseNext) dmg = Math.round(dmg * 1.1);
  if (att.shroudNext) dmg = Math.round(dmg * 1.1);
  if (att.rustNext) dmg = Math.round(dmg * 1.1);
  if (att.blindNext) dmg = Math.round(dmg * 1.1);
  if (att.crushNext) dmg = Math.round(dmg * 1.1);
  if (att.gloomedgeNext) dmg = Math.round(dmg * 1.1);
  if (att.stifleedgeNext) dmg = Math.round(dmg * 1.1);
  if (att.poxedgeNext) dmg = Math.round(dmg * 1.1);
  if (att.frailedgeNext) dmg = Math.round(dmg * 1.1);
  if (att.rootedgeNext) dmg = Math.round(dmg * 1.1);
  if (att.tangleedgeNext) dmg = Math.round(dmg * 1.1);
  if (att.glitchedgeNext) dmg = Math.round(dmg * 1.1);
  if (att.sealedgeNext) dmg = Math.round(dmg * 1.1);
  if (att.shrededgeNext) dmg = Math.round(dmg * 1.1);
  if (att.surtaxedgeNext) dmg = Math.round(dmg * 1.1);
  if (att.fearedgeNext) dmg = Math.round(dmg * 1.1);
  if (att.baneedgeNext) dmg = Math.round(dmg * 1.1);
  if (att.feebleedgeNext) dmg = Math.round(dmg * 1.1);
  if (att.silenceedgeNext) dmg = Math.round(dmg * 1.1);
  if (att.sirenedgeNext) dmg = Math.round(dmg * 1.1);
  if (att.brandedgeNext) dmg = Math.round(dmg * 1.1);
  if (att.crackedgeNext) dmg = Math.round(dmg * 1.1);
  if (att.stunedgeNext) dmg = Math.round(dmg * 1.1);
  if (att.tetheredgeNext) dmg = Math.round(dmg * 1.1);
  if (att.chokeedgeNext) dmg = Math.round(dmg * 1.1);
  if (att.huskedgeNext) dmg = Math.round(dmg * 1.1);
  if (att.festeredgeNext) dmg = Math.round(dmg * 1.1);
  if (att.drainedgeNext) dmg = Math.round(dmg * 1.1);
  if (att.bulwarkedgeNext) dmg = Math.round(dmg * 1.1);
  if (att.matadorNext) dmg = Math.round(dmg * 1.1);
  if (att.sieveNext) dmg = Math.round(dmg * 1.1);
  if ((att.skills?.usurper ?? 0) > 0 && def.def.boss && !def.phase2) dmg = Math.round(dmg * (1 + att.skills.usurper * 0.06));
  if ((att.skills?.empowered ?? 0) > 0) { const verses = (att.triumphUntilEndOfEnemyPhase ? 1 : 0) + (att.salvoUntilEndOfEnemyPhase ? 1 : 0) + (att.juggernautUntilEndOfEnemyPhase ? 1 : 0) + (att.goreUntilEndOfEnemyPhase ? 1 : 0) + (att.lanceUntilEndOfEnemyPhase ? 1 : 0) + (att.warsongUntilEndOfEnemyPhase ? 1 : 0) + (att.scorchUntilEndOfEnemyPhase ? 1 : 0) + (att.requiemUntilEndOfEnemyPhase ? 1 : 0) + (att.havocUntilEndOfEnemyPhase ? 1 : 0) + (att.crimsonUntilEndOfEnemyPhase ? 1 : 0); dmg = Math.round(dmg * (1 + att.skills.empowered * 0.04 * Math.min(3, verses))); }
  if ((att.skills?.thinner ?? 0) > 0 && !def.def.boss && !def.elite) dmg = Math.round(dmg * (1 + att.skills.thinner * 0.06));
  if ((att.skills?.purist ?? 0) > 0 && !(def.statuses?.length) && !def.exposed && !def.rended && !def.sundered && !def.crippled && !(def.doomTurns ?? 0) && !(def.sirenTurns ?? 0) && !def.cursedUntilEndOfEnemyPhase && !def.weakenUntilEndOfEnemyPhase && !def.silencedUntilEndOfEnemyPhase && !(def.rustTurns ?? 0) && !(def.stifleTurns ?? 0) && !(def.obscuredTurns ?? 0) && !(def.rotTurns ?? 0) && !(def.frailTurns ?? 0) && !(def.gloomTurns ?? 0) && !(def.poxTurns ?? 0) && !def.glitchUntilEndOfEnemyPhase && !def.mapSealUntilEndOfEnemyPhase && !def.resistSealUntilEndOfEnemyPhase && !def.baneUntilEndOfEnemyPhase) dmg = Math.round(dmg * (1 + att.skills.purist * 0.05));
  if ((att.skills?.entropist ?? 0) > 0 && ((def.doomTurns ?? 0) > 0 || (def.rustTurns ?? 0) > 0 || (def.stifleTurns ?? 0) > 0 || (def.obscuredTurns ?? 0) > 0 || (def.sirenTurns ?? 0) > 0 || (def.rotTurns ?? 0) > 0 || (def.frailTurns ?? 0) > 0 || (def.gloomTurns ?? 0) > 0 || (def.poxTurns ?? 0) > 0)) dmg = Math.round(dmg * (1 + att.skills.entropist * 0.06));
  if ((def.frailTurns ?? 0) > 0) dmg = Math.round(dmg * 1.15);
  if ((att.skills?.suffocator ?? 0) > 0 && def.en < def.def.maxEn * 0.4) dmg = Math.round(dmg * (1 + att.skills.suffocator * 0.06));
  if ((att.skills?.seaborn ?? 0) > 0 && terrainAt(map, def.pos) === 'water') dmg = Math.round(dmg * (1 + att.skills.seaborn * 0.05));
  if ((att.skills?.highlander ?? 0) > 0 && terrainAt(map, def.pos) === 'mountain') dmg = Math.round(dmg * (1 + att.skills.highlander * 0.05));
  if ((att.skills?.urbanist ?? 0) > 0 && terrainAt(map, def.pos) === 'city') dmg = Math.round(dmg * (1 + att.skills.urbanist * 0.05));
  if ((att.skills?.preyborn ?? 0) > 0 && usableWeapons(def).length === 0) dmg = Math.round(dmg * (1 + att.skills.preyborn * 0.06));
  if ((att.skills?.freefire ?? 0) > 0 && (def.silencedUntilEndOfEnemyPhase || def.counterSealUntilEndOfEnemyPhase)) dmg = Math.round(dmg * (1 + att.skills.freefire * 0.06));
  if ((att.skills?.tetherborn ?? 0) > 0 && def.tetherUntilEndOfEnemyPhase) dmg = Math.round(dmg * (1 + att.skills.tetherborn * 0.06));
  if ((att.skills?.shroudborn ?? 0) > 0 && def.shroudUntilEndOfEnemyPhase) dmg = Math.round(dmg * (1 + att.skills.shroudborn * 0.06));
  if ((att.skills?.curseborn ?? 0) > 0 && def.cursedUntilEndOfEnemyPhase) dmg = Math.round(dmg * (1 + att.skills.curseborn * 0.06));
  if ((att.skills?.nullborn ?? 0) > 0 && def.nullifiedUntilEndOfEnemyPhase) dmg = Math.round(dmg * (1 + att.skills.nullborn * 0.06));
  if ((att.skills?.dreadborn ?? 0) > 0 && def.dreadedUntilEndOfEnemyPhase) dmg = Math.round(dmg * (1 + att.skills.dreadborn * 0.06));
  if ((att.skills?.weakborn ?? 0) > 0 && def.weakenUntilEndOfEnemyPhase) dmg = Math.round(dmg * (1 + att.skills.weakborn * 0.06));
  if ((att.skills?.dischordborn ?? 0) > 0 && def.dischordUntilEndOfEnemyPhase) dmg = Math.round(dmg * (1 + att.skills.dischordborn * 0.06));
  if ((att.skills?.powerborn ?? 0) > 0 && def.en >= def.def.maxEn * 0.6) dmg = Math.round(dmg * (1 + att.skills.powerborn * 0.06));
  if ((att.skills?.winterborn ?? 0) > 0 && def.statuses?.some((fx) => fx.id === 'slow')) dmg = Math.round(dmg * (1 + att.skills.winterborn * 0.06));
  if ((att.skills?.trophyborn ?? 0) > 0 && def.elite) dmg = Math.round(dmg * (1 + att.skills.trophyborn * 0.06));
  if ((att.skills?.swiftborn ?? 0) > 0 && def.def.mobility > att.def.mobility) dmg = Math.round(dmg * (1 + att.skills.swiftborn * 0.06));
  if ((att.skills?.trampleborn ?? 0) > 0 && def.def.mobility < att.def.mobility) dmg = Math.round(dmg * (1 + att.skills.trampleborn * 0.06));
  if ((att.skills?.coreborn ?? 0) > 0 && def.pos.x > 0 && def.pos.y > 0 && def.pos.x < map.cols - 1 && def.pos.y < map.rows - 1) dmg = Math.round(dmg * (1 + att.skills.coreborn * 0.06));
  if ((att.skills?.rallyborn ?? 0) > 0 && units) { const n = units.filter((a) => a.alive && a.side === att.side && a.uid !== att.uid && dist(a.pos, att.pos) <= 1).length; dmg = Math.round(dmg * (1 + att.skills.rallyborn * 0.04 * n)); }
  if ((att.skills?.cullborn ?? 0) > 0 && def.hp <= def.def.maxHp * 0.3) dmg = Math.round(dmg * (1 + att.skills.cullborn * 0.06));
  if ((att.skills?.vengeborn ?? 0) > 0 && units) { const n = units.filter((a) => a.side === att.side && !a.alive).length; dmg = Math.round(dmg * (1 + att.skills.vengeborn * 0.04 * n)); }
  if ((att.skills?.lavaborn ?? 0) > 0 && terrainAt(map, def.pos) === 'lava') dmg = Math.round(dmg * (1 + att.skills.lavaborn * 0.05));
  if ((att.skills?.medicfoe ?? 0) > 0 && def.def.medic) dmg = Math.round(dmg * (1 + att.skills.medicfoe * 0.06));
  if ((att.skills?.packborn ?? 0) > 0 && units) { const n = units.filter((a) => a.alive && a.side === def.side && a.uid !== def.uid && dist(a.pos, def.pos) <= 1).length; dmg = Math.round(dmg * (1 + att.skills.packborn * 0.04 * n)); }
  if ((att.skills?.gauntletborn ?? 0) > 0 && units) { const n = units.filter((a) => a.alive && a.side !== att.side && dist(a.pos, att.pos) <= 1).length; dmg = Math.round(dmg * (1 + att.skills.gauntletborn * 0.04 * n)); }
  if ((att.skills?.tollborn ?? 0) > 0 && def.def.carrier) dmg = Math.round(dmg * (1 + att.skills.tollborn * 0.06));
  if ((att.skills?.massborn ?? 0) > 0 && def.def.weapons.length > att.def.weapons.length) dmg = Math.round(dmg * (1 + att.skills.massborn * 0.06));
  if ((att.skills?.scorchborn ?? 0) > 0 && def.statuses?.some((fx) => fx.id === 'burn')) dmg = Math.round(dmg * (1 + att.skills.scorchborn * 0.06));
  if ((att.skills?.hyenaborn ?? 0) > 0 && def.acted) dmg = Math.round(dmg * (1 + att.skills.hyenaborn * 0.06));
  if ((att.skills?.davidborn ?? 0) > 0 && def.level > att.level) dmg = Math.round(dmg * (1 + att.skills.davidborn * 0.06));
  if ((att.skills?.hushborn ?? 0) > 0 && def.counterSealUntilEndOfEnemyPhase) dmg = Math.round(dmg * (1 + att.skills.hushborn * 0.06));
  if ((att.skills?.voltborn ?? 0) > 0 && att.en >= att.def.maxEn * 0.6) dmg = Math.round(dmg * (1 + att.skills.voltborn * 0.05));
  if ((att.skills?.garotteborn ?? 0) > 0 && def.chokeUntilEndOfEnemyPhase) dmg = Math.round(dmg * (1 + att.skills.garotteborn * 0.06));
  if ((att.skills?.matadorborn ?? 0) > 0 && def.provokedTo != null) dmg = Math.round(dmg * (1 + att.skills.matadorborn * 0.06));
  if ((att.skills?.ironsideborn ?? 0) > 0 && armorOf(att, map) >= 600) dmg = Math.round(dmg * (1 + att.skills.ironsideborn * 0.04));
  if ((att.skills?.gloomborn ?? 0) > 0 && (def.gloomTurns ?? 0) > 0) dmg = Math.round(dmg * (1 + att.skills.gloomborn * 0.06));
  if ((att.skills?.skyborne ?? 0) > 0 && def.def.moveType === 'air') dmg = Math.round(dmg * (1 + att.skills.skyborne * 0.05));
  if ((att.skills?.rustborn ?? 0) > 0 && (def.rustTurns ?? 0) > 0) dmg = Math.round(dmg * (1 + att.skills.rustborn * 0.06));
  if ((att.skills?.rootborn ?? 0) > 0 && def.rootedUntilEndOfEnemyPhase) dmg = Math.round(dmg * (1 + att.skills.rootborn * 0.06));
  if ((att.skills?.primeborn ?? 0) > 0 && att.hp >= att.def.maxHp * 0.8) dmg = Math.round(dmg * (1 + att.skills.primeborn * 0.05));
  if ((att.skills?.spiritborn ?? 0) > 0 && att.sp >= 40) dmg = Math.round(dmg * (1 + att.skills.spiritborn * 0.05));
  if ((att.skills?.sieveborn ?? 0) > 0 && def.sieveUntilEndOfEnemyPhase) dmg = Math.round(dmg * (1 + att.skills.sieveborn * 0.06));
  if ((att.skills?.wallborn ?? 0) > 0 && Math.max(att.def.barrier ?? 0, partBonus(att, 'barrier'), att.wallUntilEndOfEnemyPhase ? 500 : 0) > 0) dmg = Math.round(dmg * (1 + att.skills.wallborn * 0.05));
  if ((att.skills?.stifleborn ?? 0) > 0 && (def.stifleTurns ?? 0) > 0) dmg = Math.round(dmg * (1 + att.skills.stifleborn * 0.06));
  if ((att.skills?.poxborn ?? 0) > 0 && (def.poxTurns ?? 0) > 0) dmg = Math.round(dmg * (1 + att.skills.poxborn * 0.06));
  if ((att.skills?.doomborn ?? 0) > 0 && (def.doomTurns ?? 0) > 0) dmg = Math.round(dmg * (1 + att.skills.doomborn * 0.06));
  if ((att.skills?.frailborn ?? 0) > 0 && (def.frailTurns ?? 0) > 0) dmg = Math.round(dmg * (1 + att.skills.frailborn * 0.06));
  if ((att.skills?.rotborn ?? 0) > 0 && (def.rotTurns ?? 0) > 0) dmg = Math.round(dmg * (1 + att.skills.rotborn * 0.06));
  if ((att.skills?.sirenborn ?? 0) > 0 && (def.sirenTurns ?? 0) > 0) dmg = Math.round(dmg * (1 + att.skills.sirenborn * 0.06));
  if ((att.skills?.veilborn ?? 0) > 0 && (def.obscuredTurns ?? 0) > 0) dmg = Math.round(dmg * (1 + att.skills.veilborn * 0.06));
  if ((att.skills?.beamfoe ?? 0) > 0 && def.def.weapons.some((w) => w.kind === 'beam')) dmg = Math.round(dmg * (1 + att.skills.beamfoe * 0.06));
  if ((att.skills?.gunfoe ?? 0) > 0 && def.def.weapons.some((w) => w.kind === 'gun')) dmg = Math.round(dmg * (1 + att.skills.gunfoe * 0.06));
  if ((att.skills?.missilefoe ?? 0) > 0 && def.def.weapons.some((w) => w.kind === 'missile')) dmg = Math.round(dmg * (1 + att.skills.missilefoe * 0.06));
  if ((att.skills?.meleefoe ?? 0) > 0 && def.def.weapons.some((w) => w.kind === 'melee')) dmg = Math.round(dmg * (1 + att.skills.meleefoe * 0.06));
  if ((att.skills?.funnelfoe ?? 0) > 0 && def.def.weapons.some((w) => w.kind === 'funnel')) dmg = Math.round(dmg * (1 + att.skills.funnelfoe * 0.06));
  if ((att.skills?.forestborn ?? 0) > 0 && terrainAt(map, def.pos) === 'forest') dmg = Math.round(dmg * (1 + att.skills.forestborn * 0.05));
  if ((att.skills?.desertborn ?? 0) > 0 && terrainAt(map, def.pos) === 'desert') dmg = Math.round(dmg * (1 + att.skills.desertborn * 0.05));
  if ((att.skills?.snowborn ?? 0) > 0 && terrainAt(map, def.pos) === 'snow') dmg = Math.round(dmg * (1 + att.skills.snowborn * 0.05));
  if ((att.skills?.ruinborn ?? 0) > 0 && terrainAt(map, def.pos) === 'ruins') dmg = Math.round(dmg * (1 + att.skills.ruinborn * 0.05));
  if ((att.skills?.voidborn ?? 0) > 0 && terrainAt(map, def.pos) === 'void') dmg = Math.round(dmg * (1 + att.skills.voidborn * 0.05));
  if ((att.skills?.willborn ?? 0) > 0 && (def.will ?? 100) >= 120) dmg = Math.round(dmg * (1 + att.skills.willborn * 0.06));
  if ((att.skills?.meekborn ?? 0) > 0 && (def.will ?? 100) <= 60) dmg = Math.round(dmg * (1 + att.skills.meekborn * 0.06));
  if ((att.skills?.huskborn ?? 0) > 0 && def.huskUntilEndOfEnemyPhase) dmg = Math.round(dmg * (1 + att.skills.huskborn * 0.06));
  if ((att.skills?.rimborn ?? 0) > 0 && (def.pos.x === 0 || def.pos.y === 0 || def.pos.x === map.cols - 1 || def.pos.y === map.rows - 1)) dmg = Math.round(dmg * (1 + att.skills.rimborn * 0.06));
  if ((att.skills?.farborn ?? 0) > 0 && dist(att.pos, def.pos) >= 6) dmg = Math.round(dmg * (1 + att.skills.farborn * 0.06));
  if ((att.skills?.spentborn ?? 0) > 0 && def.en <= def.def.maxEn * 0.4) dmg = Math.round(dmg * (1 + att.skills.spentborn * 0.06));
  if ((att.skills?.festerborn ?? 0) > 0 && def.festerUntilEndOfEnemyPhase) dmg = Math.round(dmg * (1 + att.skills.festerborn * 0.06));
  if ((att.skills?.baseborn ?? 0) > 0 && terrainAt(map, def.pos) === 'base') dmg = Math.round(dmg * (1 + att.skills.baseborn * 0.05));
  if ((att.skills?.roadborn ?? 0) > 0 && terrainAt(map, def.pos) === 'road') dmg = Math.round(dmg * (1 + att.skills.roadborn * 0.05));
  if ((att.skills?.sniperfoe ?? 0) > 0 && def.def.weapons.some((w) => w.sniper)) dmg = Math.round(dmg * (1 + att.skills.sniperfoe * 0.06));
  if ((att.skills?.moonborn ?? 0) > 0 && terrainAt(map, def.pos) === 'moon') dmg = Math.round(dmg * (1 + att.skills.moonborn * 0.05));
  if (units && (att.skills?.clusterborn ?? 0) > 0 && units.filter((a) => a.alive && a.side === def.side && a.uid !== def.uid && dist(a.pos, def.pos) <= 1).length >= 2) dmg = Math.round(dmg * (1 + att.skills.clusterborn * 0.04));
  if ((att.skills?.knockfoe ?? 0) > 0 && def.def.weapons.some((w) => w.knockback)) dmg = Math.round(dmg * (1 + att.skills.knockfoe * 0.06));
  if ((att.skills?.drainfoe ?? 0) > 0 && def.def.weapons.some((w) => w.drain)) dmg = Math.round(dmg * (1 + att.skills.drainfoe * 0.06));
  if ((att.skills?.piercefoe ?? 0) > 0 && def.def.weapons.some((w) => w.pierce)) dmg = Math.round(dmg * (1 + att.skills.piercefoe * 0.06));
  if ((att.skills?.chainfoe ?? 0) > 0 && def.def.weapons.some((w) => w.chain != null)) dmg = Math.round(dmg * (1 + att.skills.chainfoe * 0.06));
  if ((att.skills?.breakerfoe ?? 0) > 0 && def.def.weapons.some((w) => w.breaker)) dmg = Math.round(dmg * (1 + att.skills.breakerfoe * 0.06));
  if ((att.skills?.volleyfoe ?? 0) > 0 && def.def.weapons.some((w) => (w.multiHit ?? 0) > 1)) dmg = Math.round(dmg * (1 + att.skills.volleyfoe * 0.06));
  if ((att.skills?.acefoe ?? 0) > 0 && def.def.weapons.some((w) => w.aceReq != null)) dmg = Math.round(dmg * (1 + att.skills.acefoe * 0.06));
  if ((att.skills?.hexfoe ?? 0) > 0 && def.def.weapons.some((w) => w.status != null)) dmg = Math.round(dmg * (1 + att.skills.hexfoe * 0.06));
  if ((att.skills?.wingfoe ?? 0) > 0 && def.def.weapons.some((w) => w.antiAir)) dmg = Math.round(dmg * (1 + att.skills.wingfoe * 0.06));
  if ((att.skills?.skirmfoe ?? 0) > 0 && def.def.weapons.some((w) => w.postMove)) dmg = Math.round(dmg * (1 + att.skills.skirmfoe * 0.06));
  if ((att.skills?.willfoe ?? 0) > 0 && def.def.weapons.some((w) => w.willReq)) dmg = Math.round(dmg * (1 + att.skills.willfoe * 0.06));
  if ((att.skills?.mapfoe ?? 0) > 0 && def.def.weapons.some((w) => w.mapRange != null)) dmg = Math.round(dmg * (1 + att.skills.mapfoe * 0.06));
  if ((att.skills?.voltfoe ?? 0) > 0 && def.def.weapons.some((w) => (w.enCost ?? 0) >= 20)) dmg = Math.round(dmg * (1 + att.skills.voltfoe * 0.06));
  if ((att.skills?.shellfoe ?? 0) > 0 && def.def.weapons.some((w) => w.ammo != null)) dmg = Math.round(dmg * (1 + att.skills.shellfoe * 0.06));
  if ((att.skills?.truefoe ?? 0) > 0 && def.def.weapons.some((w) => w.hitMod >= 10)) dmg = Math.round(dmg * (1 + att.skills.truefoe * 0.06));
  if ((att.skills?.heavyfoe ?? 0) > 0 && def.def.weapons.some((w) => w.power >= 4000)) dmg = Math.round(dmg * (1 + att.skills.heavyfoe * 0.06));
  if ((att.skills?.critfoe ?? 0) > 0 && def.def.weapons.some((w) => (w.critMod ?? 0) >= 15)) dmg = Math.round(dmg * (1 + att.skills.critfoe * 0.06));
  if ((att.skills?.cannonfoe ?? 0) > 0 && def.def.weapons.some((w) => w.rangeMax >= 7)) dmg = Math.round(dmg * (1 + att.skills.cannonfoe * 0.06));
  if ((att.skills?.skyfoe ?? 0) > 0 && def.def.moveType === 'air') dmg = Math.round(dmg * (1 + att.skills.skyfoe * 0.06));
  if ((att.skills?.regifoe ?? 0) > 0 && def.def.boss) dmg = Math.round(dmg * (1 + att.skills.regifoe * 0.06));
  if ((att.skills?.ambushborn ?? 0) > 0 && def.hp >= def.def.maxHp * 0.8) dmg = Math.round(dmg * (1 + att.skills.ambushborn * 0.06));
  if ((att.skills?.underborn ?? 0) > 0 && att.hp <= att.def.maxHp * 0.3) dmg = Math.round(dmg * (1 + att.skills.underborn * 0.06));
  if ((att.skills?.breakborn ?? 0) > 0 && (def.statuses ?? []).some((x) => x.id === 'break')) dmg = Math.round(dmg * (1 + att.skills.breakborn * 0.06));
  if ((att.skills?.stunborn ?? 0) > 0 && (def.statuses ?? []).some((x) => x.id === 'stun')) dmg = Math.round(dmg * (1 + att.skills.stunborn * 0.06));
  if ((att.skills?.suppborn ?? 0) > 0 && (def.statuses ?? []).some((x) => x.id === 'supp')) dmg = Math.round(dmg * (1 + att.skills.suppborn * 0.06));
  if ((att.skills?.markborn ?? 0) > 0 && (def.statuses ?? []).some((x) => x.id === 'mark')) dmg = Math.round(dmg * (1 + att.skills.markborn * 0.06));
  if (units && (att.skills?.loneborn ?? 0) > 0 && units.filter((a) => a.alive && a.side === def.side && a.uid !== def.uid && dist(a.pos, def.pos) <= 2).length === 0) dmg = Math.round(dmg * (1 + att.skills.loneborn * 0.06));
  if ((att.skills?.eliteborn ?? 0) > 0 && def.elite) dmg = Math.round(dmg * (1 + att.skills.eliteborn * 0.06));
  if ((att.skills?.fuelborn ?? 0) > 0 && att.en >= att.def.maxEn * 0.75) dmg = Math.round(dmg * (1 + att.skills.fuelborn * 0.06));
  if ((att.skills?.dryborn ?? 0) > 0 && def.def.weapons.some((x) => x.ammo != null) && def.def.weapons.every((x) => x.ammo == null || (def.ammo[x.id] ?? 0) <= 0)) dmg = Math.round(dmg * (1 + att.skills.dryborn * 0.06));
  if ((att.skills?.plagueborn ?? 0) > 0 && ((def.statuses?.length ?? 0) + (def.doomTurns ? 1 : 0) + (def.sirenTurns ? 1 : 0) + (def.exposed ? 1 : 0) + (def.rended ? 1 : 0) + (def.sundered ? 1 : 0) + (def.crippled ? 1 : 0) + (def.silencedUntilEndOfEnemyPhase ? 1 : 0) + (def.cursedUntilEndOfEnemyPhase ? 1 : 0) + (def.weakenUntilEndOfEnemyPhase ? 1 : 0) + (def.obscuredTurns ? 1 : 0) + (def.nullifiedUntilEndOfEnemyPhase ? 1 : 0) + (def.tetherUntilEndOfEnemyPhase ? 1 : 0) + (def.rustTurns ? 1 : 0) + (def.stifleTurns ? 1 : 0) + (def.rotTurns ? 1 : 0) + (def.frailTurns ? 1 : 0) + (def.gloomTurns ? 1 : 0) + (def.poxTurns ? 1 : 0) + (def.glitchUntilEndOfEnemyPhase ? 1 : 0) + (def.mapSealUntilEndOfEnemyPhase ? 1 : 0) + (def.resistSealUntilEndOfEnemyPhase ? 1 : 0) + (def.baneUntilEndOfEnemyPhase ? 1 : 0)) >= 3) dmg = Math.round(dmg * (1 + att.skills.plagueborn * 0.04));
  if ((att.skills?.anchorborn ?? 0) > 0 && (att.anchoredUntilEndOfEnemyPhase || partBonus(att, 'knockProof') > 0)) dmg = Math.round(dmg * (1 + att.skills.anchorborn * 0.05));
  ...
  if (def.flakUntilEndOfEnemyPhase && dist(att.pos, def.pos) > 1) dmg = Math.round(dmg * 0.8);
  if (def.bulwarkUntilEndOfEnemyPhase && dist(att.pos, def.pos) <= 1) dmg = Math.round(dmg * 0.8);
  ...
  if (w.kind === 'beam' && att.beamverseUntilEndOfEnemyPhase) dmg = Math.round(dmg * 1.15);
  if (w.kind === 'melee' && att.warlordUntilEndOfEnemyPhase) dmg = Math.round(dmg * 1.15);
  if (w.kind === 'missile' && att.shellUntilEndOfEnemyPhase) dmg = Math.round(dmg * 1.15);
  if (w.kind === 'gun' && att.gunUntilEndOfEnemyPhase) dmg = Math.round(dmg * 1.15);
  if (w.kind === 'funnel' && att.funnelUntilEndOfEnemyPhase) dmg = Math.round(dmg * 1.15);
  ...
  if ((att.skills?.zenborn ?? 0) > 0 && (w.willReq ?? 0) > 0) dmg = Math.round(dmg * (1 + att.skills.zenborn * 0.06));
  if ((att.skills?.flakborn ?? 0) > 0 && def.def.weapons.some((x) => x.mapRange != null)) dmg = Math.round(dmg * (1 + att.skills.flakborn * 0.05));
  ...
  if (def.level < att.level && (att.skills?.overlord ?? 0) > 0) dmg = Math.round(dmg * (1 + att.skills.overlord * 0.05));
  if ((att.skills?.volleyborn ?? 0) > 0 && (w.multiHit ?? 1) > 1) dmg = Math.round(dmg * (1 + att.skills.volleyborn * 0.06));
  ...
  const res = att.voidedgeNext || def.resistSealUntilEndOfEnemyPhase ? 0 : (def.def.resists?.[w.kind] ?? 0);
  ...
  if (w.mapRange != null && att.bombardverseUntilEndOfEnemyPhase) dmg = Math.round(dmg * 1.2);
  ...
  const barrier = (att.pierceNext || att.piercebrandUntilEndOfEnemyPhase || def.sieveUntilEndOfEnemyPhase) ? 0 : Math.max(def.def.barrier ?? 0, partBonus(def, 'barrier'), def.wallUntilEndOfEnemyPhase ? 500 : 0, def.haloUntilEndOfEnemyPhase ? 400 : 0);
  ...
  if (def.damperUntilEndOfEnemyPhase && dmg > 3000) dmg = 3000;
  return dmg;
}

const rnd = () => Math.random() * 100;
export const critChance = (att: UnitState, def: UnitState, w?: WeaponDef) => def.aegisUntilEndOfEnemyPhase || def.steadfastUntilEndOfEnemyPhase || att.nullifiedUntilEndOfEnemyPhase || partBonus(def, 'critGuard') > 0 ? 0 : Math.max(5, Math.round(8 + (att.def.mobility - def.def.mobility) * 0.2 + (w?.critMod ?? 0) + partBonus(att, 'crit') + (att.charged ? 15 : 0) + (att.marksmanUntilEndOfEnemyPhase ? 15 : 0) + (att.havocUntilEndOfEnemyPhase ? 15 : 0) + (att.surgeUntilEndOfEnemyPhase ? 15 : 0) + (att.keenUntilEndOfEnemyPhase ? 10 : 0) + (att.enraged ? 10 : 0) + (att.skills?.crit ?? 0) * 2 + (def.baneUntilEndOfEnemyPhase ? 25 : 0)));
...
function resolveHit(att: UnitState, def: UnitState, w: WeaponDef, map: MapDef, mods: CombatMods = NO_MODS, units?: UnitState[]): SimAttack {
  ...
  const strike = (remainingHp: number): SimAttack => {
    ...
    if (att.glitchUntilEndOfEnemyPhase) {
      const g = Math.round(damageOf(att, def, w, map, false, mods.dmgMult, units) * 0.45);
      return { hit: true, crit: false, graze: true, damage: g, hitChance: hc, destroyed: remainingHp - g <= 0 };
    }
...
export function bestCounterWeapon(def: UnitState, attPos: Pos): WeaponDef | undefined {
  ...
  const opts = usableWeapons(def).filter((w) => dist(def.pos, attPos) >= w.rangeMin && dist(def.pos, attPos) <= rangeMaxOf(def, w) + partBonus(def, 'counterRange') + (def.repulseUntilEndOfEnemyPhase ? 1 : 0) - (def.chokeUntilEndOfEnemyPhase ? 1 : 0) && !w.mapRange);
...
export function simulateAttack(att: UnitState, def: UnitState, w: WeaponDef, map: MapDef, attMods: CombatMods = NO_MODS, defMods: CombatMods = NO_MODS, reaction: Reaction = 'counter', units?: UnitState[]): AttackResult {
  ...
  if (reaction === 'counter' && !att.overrunNext && !att.ghostNext && !att.phantomUntilEndOfEnemyPhase && !def.counterSealUntilEndOfEnemyPhase) {
    ...
    if (cw && cutRank > 0 && Math.random() * 100 < cutRank * 4) {
      ...
      if (def.riposteUntilEndOfEnemyPhase) c.damage = Math.round(c.damage * 1.15);
      ...
      if (def.feebleUntilEndOfEnemyPhase) c.damage = Math.round(c.damage * 0.75);
...
export function applyAttack(state: GameState, attackerUid: string, defenderUid: string, weaponId: string, mods?: (u: UnitState) => CombatMods, reaction: Reaction = 'counter'): { state: GameState; result: AttackResult } {
  ...
  if (result.hit && def.alive && (partBonus(def, 'reflect') > 0 || def.mirrorwallUntilEndOfEnemyPhase || def.thornUntilEndOfEnemyPhase)) {
    const rd = Math.min(att.hp - 1, Math.round(result.damage * (partBonus(def, 'reflect') > 0 ? partBonus(def, 'reflect') / 100 : 0.1)));
    if (rd > 0) {
      att.hp -= rd;
      result.expEvents.push(`\u21C4 ${def.thornUntilEndOfEnemyPhase ? 'THORN VERSE' : 'REACTIVE ARMOR'} — ${att.def.name} takes ${rd} backlash`);
    }
  }
  ...
  if (result.hit && def.alive && att.muteNext && !result.graze) def.silencedUntilEndOfEnemyPhase = true;
  ...
  if (result.hit && att.leechUntilEndOfEnemyPhase && !result.graze) att.hp = Math.min(att.def.maxHp, att.hp + Math.round(result.damage * 0.10));
  ...
  // harpoon drag — a landed hit hauls the survivor one tile toward the striker
  if (result.hit && def.alive && att.curseNext && !result.graze) def.cursedUntilEndOfEnemyPhase = true;
  if (result.hit && def.alive && att.shroudNext && !result.graze) def.shroudUntilEndOfEnemyPhase = true;
  if (result.hit && def.alive && att.rustNext && !result.graze) def.rustTurns = 2;
  if (result.hit && def.alive && att.blindNext && !result.graze) def.nullifiedUntilEndOfEnemyPhase = true;
  if (result.hit && def.alive && att.crushNext && !result.graze) applyStatus(def, 'break');
  if (result.hit && def.alive && att.gloomedgeNext && !result.graze) def.gloomTurns = 2;
  if (result.hit && def.alive && att.stifleedgeNext && !result.graze) def.stifleTurns = 2;
  if (result.hit && def.alive && att.poxedgeNext && !result.graze) def.poxTurns = 2;
  if (result.hit && def.alive && att.frailedgeNext && !result.graze) def.frailTurns = 2;
  if (result.hit && def.alive && att.rootedgeNext && !result.graze) def.rootedUntilEndOfEnemyPhase = true;
  if (result.hit && def.alive && att.tangleedgeNext && !result.graze) def.tangleUntilEndOfEnemyPhase = true;
  if (result.hit && def.alive && att.glitchedgeNext && !result.graze) def.glitchUntilEndOfEnemyPhase = true;
  if (result.hit && def.alive && att.sealedgeNext && !result.graze) def.mapSealUntilEndOfEnemyPhase = true;
  if (result.hit && def.alive && att.shrededgeNext && !result.graze) def.resistSealUntilEndOfEnemyPhase = true;
  if (result.hit && def.alive && att.surtaxedgeNext && !result.graze) def.surtaxUntilEndOfEnemyPhase = true;
  if (result.hit && def.alive && att.emberUntilEndOfEnemyPhase && !result.graze) applyStatus(def, 'burn');
  if (result.hit && def.alive && att.markbrandUntilEndOfEnemyPhase && !result.graze) applyStatus(def, 'mark');
  if (result.hit && def.alive && att.breakbrandUntilEndOfEnemyPhase && !result.graze) applyStatus(def, 'break');
  if (result.hit && def.alive && att.slowbrandUntilEndOfEnemyPhase && !result.graze) applyStatus(def, 'slow');
  if (result.hit && def.alive && att.suppbrandUntilEndOfEnemyPhase && !result.graze) applyStatus(def, 'supp');
  if (result.hit && def.alive && att.stunbrandUntilEndOfEnemyPhase && !result.graze) applyStatus(def, 'stun');
  if (result.hit && def.alive && att.banebrandUntilEndOfEnemyPhase && !result.graze) def.baneUntilEndOfEnemyPhase = true;
  if (result.hit && def.alive && att.fearedgeNext && !result.graze) def.weakenUntilEndOfEnemyPhase = true;
  if (result.hit && def.alive && att.baneedgeNext && !result.graze) def.baneUntilEndOfEnemyPhase = true;
  if (result.hit && def.alive && att.feebleedgeNext && !result.graze) def.feebleUntilEndOfEnemyPhase = true;
  if (result.hit && def.alive && att.silenceedgeNext && !result.graze) def.silencedUntilEndOfEnemyPhase = true;
  if (result.hit && def.alive && att.sirenedgeNext && !result.graze) def.sirenTurns = 2;
  if (result.hit && def.alive && att.brandedgeNext && !result.graze) applyStatus(def, 'mark');
  if (result.hit && def.alive && att.crackedgeNext && !result.graze) applyStatus(def, 'break');
  if (result.hit && def.alive && att.stunedgeNext && !result.graze) applyStatus(def, 'stun');
  if (result.hit && def.alive && att.tetheredgeNext && !result.graze) def.tetherUntilEndOfEnemyPhase = true;
  if (result.hit && def.alive && att.chokeedgeNext && !result.graze) def.chokeUntilEndOfEnemyPhase = true;
  if (result.hit && def.alive && att.huskedgeNext && !result.graze) def.huskUntilEndOfEnemyPhase = true;
  if (result.hit && def.alive && att.festeredgeNext && !result.graze) def.festerUntilEndOfEnemyPhase = true;
  if (result.hit && def.alive && att.drainedgeNext && !result.graze) def.en = Math.max(0, def.en - 20);
  if (result.hit && att.alive && att.bulwarkedgeNext && !result.graze) att.wallUntilEndOfEnemyPhase = true;
  if (result.hit && def.alive && att.matadorNext && !result.graze) def.provokedTo = att.uid;
  if (result.hit && def.alive && att.sieveNext && !result.graze) def.sieveUntilEndOfEnemyPhase = true;
  if (result.hit && def.alive && !result.destroyed && att.dragNext && partBonus(def, 'knockProof') === 0 && !def.anchoredUntilEndOfEnemyPhase && dist(def.pos, att.pos) > 1) {
    const px = Math.sign(att.pos.x - def.pos.x);
    const py = Math.sign(att.pos.y - def.pos.y);
    const opts =
      Math.abs(def.pos.x - att.pos.x) >= Math.abs(def.pos.y - att.pos.y)
        ? [{ x: def.pos.x + px, y: def.pos.y }, { x: def.pos.x, y: def.pos.y + py }]
        : [{ x: def.pos.x, y: def.pos.y + py }, { x: def.pos.x + px, y: def.pos.y }];
    for (const np of opts) {
      if (np.x === def.pos.x && np.y === def.pos.y) continue;
      if (np.x < 0 || np.y < 0 || np.x >= state.map.cols || np.y >= state.map.rows) continue;
      if (!TERRAIN_INFO[terrainAt(state.map, np)].passable[def.def.moveType]) continue;
  ...
  // blink edge — a landed hit teleports the striker to a free tile beside the target
  if (result.hit && def.alive && !result.destroyed && att.blinkNext && dist(att.pos, def.pos) > 1) {
    const opts = [
      { x: def.pos.x + 1, y: def.pos.y }, { x: def.pos.x - 1, y: def.pos.y },
      { x: def.pos.x, y: def.pos.y + 1 }, { x: def.pos.x, y: def.pos.y - 1 },
    ].sort((a, b) => dist(a, att.pos) - dist(b, att.pos));
    for (const np of opts) {
      if (np.x < 0 || np.y < 0 || np.x >= state.map.cols || np.y >= state.map.rows) continue;
      if (!TERRAIN_INFO[terrainAt(state.map, np)].passable[att.def.moveType]) continue;
      if (state.units.some((o) => o.alive && o !== att && o.pos.x === np.x && o.pos.y === np.y)) continue;
      att.pos = np;
      break;
    }
  }
  ...
  att.moved = true;
  att.acted = !(result.destroyed && att.reshiftNext);
  ...
  att.pyreNext = false; att.cinderNext = false; att.maraudNext = false; att.howlNext = false; att.vampNext = false; att.disarmNext = false; att.twinNext = false; att.pierceNext = false; att.reshiftNext = false; att.muteNext = false; att.dragNext = false; att.blinkNext = false; att.curseNext = false; att.shroudNext = false; att.rustNext = false; att.blindNext = false; att.crushNext = false; att.gloomedgeNext = false; att.matadorNext = false; att.sieveNext = false; att.stifleedgeNext = false; att.poxedgeNext = false; att.frailedgeNext = false; att.rootedgeNext = false; att.tangleedgeNext = false; att.glitchedgeNext = false; att.sealedgeNext = false; att.shrededgeNext = false; att.surtaxedgeNext = false; att.fearedgeNext = false; att.baneedgeNext = false; att.feebleedgeNext = false; att.silenceedgeNext = false; att.sirenedgeNext = false; att.brandedgeNext = false; att.crackedgeNext = false; att.stunedgeNext = false; att.tetheredgeNext = false; att.chokeedgeNext = false; att.huskedgeNext = false; att.festeredgeNext = false; att.drainedgeNext = false; att.bulwarkedgeNext = false;
...
function awardExp(u: UnitState, amount: number, events: string[]) {
  ...
  amount = Math.round(amount * (1 + partBonus(u, 'xp') / 100 + (u.fableUntilEndOfEnemyPhase ? 0.5 : 0)));
...
export function applySpirit(u: UnitState, spirit: SpiritId): void {
  ...
  switch (spirit) {
    ...
    case 'vaultverse':
      break;
    case 'shroudedge': u.shroudNext = true; break;
    case 'freeflowverse':
      break;
    case 'rustedge': u.rustNext = true; break;
    case 'blindedge': u.blindNext = true; break;
    case 'crushedge': u.crushNext = true; break;
    case 'gloomedge': u.gloomedgeNext = true; break;
    case 'matadoredge': u.matadorNext = true; break;
    case 'feebleverse':
      break;
    case 'festerverse':
      break;
    ...
    case 'revelverse':
      break;
    case 'sieveedge': u.sieveNext = true; break;
    case 'bulwarkverse': break;
    case 'stifleedge': u.stifleedgeNext = true; break;
    case 'poxverse': break;
    case 'shatterverse': break;
    case 'anchorverse': break;
    case 'poxedge': u.poxedgeNext = true; break;
    case 'frailedge': u.frailedgeNext = true; break;
    case 'rootedge': u.rootedgeNext = true; break;
    case 'tangleedge': u.tangleedgeNext = true; break;
    case 'sealverse': break;
    case 'zeroverse': break;
    case 'baneverse': break;
    case 'duskverse': break;
    case 'reactorverse': break;
    case 'gunverse': break;
    case 'glitchedge': u.glitchedgeNext = true; break;
    case 'funnelverse': break;
    case 'sealedge': u.sealedgeNext = true; break;
    case 'shrededge': u.shrededgeNext = true; break;
    case 'haloverse': break;
    case 'surtaxedge': u.surtaxedgeNext = true; break;
    case 'viceverse': break;
    case 'drainverse': break;
    case 'emberverse': break;
    case 'fearedge': u.fearedgeNext = true; break;
    case 'bombardverse': break;
    case 'baneedge': u.baneedgeNext = true; break;
    case 'cowardverse': break;
    case 'nadirverse': break;
    case 'hasteverse': break;
    case 'feebleedge': u.feebleedgeNext = true; break;
    case 'riposteverse': break;
    case 'silenceedge': u.silenceedgeNext = true; break;
    case 'brandverse': break;
    case 'suppressverse': break;
    case 'keenverse': break;
    case 'sirenedge': u.sirenedgeNext = true; break;
    case 'arsenalverse': break;
    case 'brandedge': u.brandedgeNext = true; break;
    case 'muteverse': break;
    case 'hazeverse': break;
    case 'huntverse': break;
    case 'crackedge': u.crackedgeNext = true; break;
    case 'stunedge': u.stunedgeNext = true; break;
    case 'tetheredge': u.tetheredgeNext = true; break;
    case 'chokeedge': u.chokeedgeNext = true; break;
    case 'siegeverse': break;
    case 'vexverse': break;
    case 'pealverse': break;
    case 'huskedge': u.huskedgeNext = true; break;
    case 'pierceverse': break;
    case 'festeredge': u.festeredgeNext = true; break;
    case 'drainedge': u.drainedgeNext = true; break;
    case 'bulwarkedge': u.bulwarkedgeNext = true; break;
    case 'snareverse': break;
    case 'quietusverse': break;
    case 'bountyverse': break;
    case 'graveverse': break;
    case 'wraithverse': break;
    case 'abyssverse': break;
    case 'blitzverse': break;
    case 'carnageverse': break;
    case 'rendverse': break;
    case 'oblivionverse': break;
    case 'jinxverse': break;
    case 'rimeverse': break;
    case 'staticverse': break;
    case 'riftverse': break;
    case 'glitchverse': break;
    case 'warlordverse': break;
    case 'shellverse': break;
    case 'beamverse': break;
    case 'defianceverse':
      break;
    case 'havocverse':
      break;
...
export function clearTransientForOwnPhase(u: UnitState): void {
  ...
  u.lanceUntilEndOfEnemyPhase = false;
  u.terraUntilEndOfEnemyPhase = false;
  ...
  for (const e of enemies) {
    ...
    for (const tile of tiles) {
      for (const w of usableWeapons(e)) {
        if (w.mapRange == null || e.mapSealUntilEndOfEnemyPhase) continue;
...
export function phaseRecovery(u: UnitState, map: MapDef, units?: UnitState[]): { hpGain: number; enGain: number; hpLoss: number } {
  ...
  if (u.poxTurns) { u.poxTurns -= 1; u.hp = Math.max(1, u.hp - Math.round(u.def.maxHp * 0.05)); }
  if (u.rotTurns) u.rotTurns -= 1;
  if (u.frailTurns) u.frailTurns -= 1;
  if (u.gloomTurns) u.gloomTurns -= 1;
  if (u.mendTurns) u.mendTurns -= 1;
  if (u.doomTurns) u.doomTurns -= 1;
  const hpLoss = partBonus(u, 'terraProof') > 0 || u.terraUntilEndOfEnemyPhase ? 0 : Math.min(u.hp - 1, Math.round(u.def.maxHp * (t.hpDmg ?? 0))) + (u.doomTurns ? Math.min(u.hp - 1, Math.round(u.def.maxHp * 0.08)) : 0); // terrain & decay can't kill — leaves 1 HP
  if (partBonus(u, 'ammoRegen') > 0 || u.sustainUntilEndOfEnemyPhase) {
    for (const w of u.def.weapons) {
      if (w.ammo != null) u.ammo[w.id] = Math.min(maxAmmoOf(u, w), (u.ammo[w.id] ?? 0) + 1);
    }
  }
  u.en += enGain;
  u.sp = Math.min(u.def.pilot.maxSp, u.sp + partBonus(u, 'spRegen') + (u.skills?.savant ?? 0) + (u.revelUntilEndOfEnemyPhase ? 3 : 0));
