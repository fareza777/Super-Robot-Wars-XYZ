...
function evadeOf(u: UnitState, map: MapDef): number {
  ...
  return u.def.mobility + u.def.pilot.evade + (u.level - 1) * 2 + t.eva + (u.focusUntilEndOfEnemyPhase ? 30 : 0) + (u.veilUntilEndOfEnemyPhase ? 20 : 0) + (u.hymnUntilEndOfEnemyPhase ? 15 : 0) + willEvade(u) + partBonus(u, 'mobility') + partBonus(u, 'evade') * 1.8 + (u.skills?.evade ?? 0) - (u.skills?.gambit ?? 0) * 8 + (u.aceMastery ? 5 : 0) + (u.skills?.phantomstep ?? 0) * 4 - (u.dodges ?? 0) * 8 - (u.sundered ? 15 : 0) - (u.dischordUntilEndOfEnemyPhase ? 15 : 0);
}

function armorOf(u: UnitState, map: MapDef): number {
  const t = TERRAIN_INFO[terrainAt(map, u.pos)];
  const base = u.def.armor + (u.level - 1) * 40 + t.def + (u.gritUntilEndOfEnemyPhase ? 400 : 0) + (partBonus(u, 'lowHpArmor') > 0 && u.hp < u.def.maxHp * 0.4 ? 400 : 0) + (u.sanctumUntilEndOfEnemyPhase ? 400 : 0) + (u.oathUntilEndOfEnemyPhase ? 600 : 0) + (u.skills?.ironbound ?? 0) * 100 + willArmor(u) + partBonus(u, 'armor') + (u.phase2 ? 300 : 0) + (t.def > 0 ? partBonus(u, 'fortArmor') : 0);
...
export function hitChance(att: UnitState, def: UnitState, w: WeaponDef, map: MapDef, hitBonus = 0): number {
  ...
  const raw = 72 + statFor(att, w) * 0.6 + w.hitMod + att.def.mobility * 0.25 + hitBonus + traitHit + willHitBonus(att) + partBonus(att, 'hit') + (att.skills?.hit ?? 0) + (att.aceMastery ? 5 : 0) + (att.hymnUntilEndOfEnemyPhase ? 15 : 0) + (att.absolveUntilEndOfEnemyPhase ? 15 : 0) + (att.bannerUntilEndOfEnemyPhase ? 10 : 0) + (att.aimed ? 15 + partBonus(att, 'aimBoost') : 0) + (att.exertNext ? 20 : 0) + (def.exposed ? 20 : 0) + (def.statuses?.some((fx) => fx.id === 'mark') ? 25 : 0) - (att.statuses?.some((fx) => fx.id === 'supp') ? 20 : 0) - partBonus(def, 'chaff') - evadeOf(def, map) * 0.55;
...
export function applyAttack(state: GameState, attackerUid: string, defenderUid: string, weaponId: string, mods?: (u: UnitState) => CombatMods, reaction: Reaction = 'counter'): { state: GameState; result: AttackResult } {
  ...
  if (result.hit && def.alive && !result.destroyed && (w.knockback || att.knockNext) && partBonus(def, 'knockProof') === 0) {
  ...
  att.judgeNext = false;
  att.goreNext = false; att.rampageNext = false; att.hemoNext = false; att.voidedgeNext = false; att.plunderNext = false; att.knockNext = false; att.lacerateNext = false;
...
export function phaseRecovery(u: UnitState, map: MapDef, units?: UnitState[]): { hpGain: number; enGain: number; hpLoss: number } {
  ...
  const hpGain = Math.min(u.def.maxHp - u.hp, Math.round(u.def.maxHp * (t.hpRegen ?? 0)) + Math.round(u.def.maxHp * (partBonus(u, 'hpRegen') / 100)) + Math.round(u.def.maxHp * 0.01 * (u.skills?.regen ?? 0)) + Math.round(u.def.maxHp * healPct / 100) + (partBonus(u, 'lowHpRegen') > 0 && u.hp < u.def.maxHp * 0.4 ? Math.round(u.def.maxHp * 0.08) : 0));
