...
export function damageOf(att: UnitState, def: UnitState, w: WeaponDef, map: MapDef, crit: boolean, dmgMult = 1, units?: UnitState[]): number {
  ...
  if ((att.skills?.purist ?? 0) > 0 && !(def.statuses?.length) && !def.exposed && !def.rended && !def.sundered && !def.crippled && !(def.doomTurns ?? 0) && !(def.sirenTurns ?? 0) && !def.cursedUntilEndOfEnemyPhase && !def.weakenUntilEndOfEnemyPhase && !def.silencedUntilEndOfEnemyPhase && !(def.rustTurns ?? 0) && !(def.stifleTurns ?? 0) && !(def.obscuredTurns ?? 0) && !(def.rotTurns ?? 0) && !(def.frailTurns ?? 0) && !(def.gloomTurns ?? 0)) dmg = Math.round(dmg * (1 + att.skills.purist * 0.05));
  if ((att.skills?.entropist ?? 0) > 0 && ((def.doomTurns ?? 0) > 0 || (def.rustTurns ?? 0) > 0 || (def.stifleTurns ?? 0) > 0 || (def.obscuredTurns ?? 0) > 0 || (def.sirenTurns ?? 0) > 0 || (def.rotTurns ?? 0) > 0 || (def.frailTurns ?? 0) > 0 || (def.gloomTurns ?? 0) > 0)) dmg = Math.round(dmg * (1 + att.skills.entropist * 0.06));
