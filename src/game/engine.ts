...
export function enCostOf(u: UnitState, w: WeaponDef): number {
  ...
  const mult = (1 - 0.04 * r) * (1 - p / 100) * rage;
...
export const critChance = (att: UnitState, def: UnitState, w?: WeaponDef) => def.aegisUntilEndOfEnemyPhase ? 0 : Math.max(5, Math.round(8 + (att.def.mobility - def.def.mobility) * 0.2 + (w?.critMod ?? 0) + partBonus(att, 'crit') + (att.charged ? 15 : 0) + (att.marksmanUntilEndOfEnemyPhase ? 15 : 0) + (att.enraged ? 10 : 0) + (att.skills?.crit ?? 0) * 2));
...
export function applyAttack(state: GameState, attackerUid: string, defenderUid: string, weaponId: string, mods?: (u: UnitState) => CombatMods, reaction: Reaction = 'counter'): { state: GameState; result: AttackResult } {
  ...
  att.judgeNext = false;
  att.goreNext = false; att.rampageNext = false; att.hemoNext = false; att.voidedgeNext = false; att.plunderNext = false;
