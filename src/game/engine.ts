...
export function moveRangeOf(u: UnitState): number {
  return Math.max(1, u.def.moveRange + (u.accelThisTurn ?? 0) + partBonus(u, 'move') + (u.tideUntilEndOfEnemyPhase ? 1 : 0) + (u.def.moveType === 'air' ? partBonus(u, 'skyBooster') : 0) + (u.en > u.def.maxEn * 0.75 ? partBonus(u, 'pulseVernier') : 0) + (u.def.moveType === 'land' ? partBonus(u, 'landVernier') : 0) - (u.ebbUntilEndOfEnemyPhase ? 1 : 0) - (u.crippled ? 2 : 0) - (u.statuses?.some((fx) => fx.id === 'slow') ? 3 : 0));
}
...
export function armorOf(u: UnitState, map: MapDef): number {
  const t = TERRAIN_INFO[terrainAt(map, u.pos)];
  const base = u.def.armor + (u.level - 1) * 40 + t.def + (u.en > u.def.maxEn * 0.5 ? partBonus(u, 'reactorShield') : 0) - (partBonus(u, 'ventArmor') > 0 ? 100 : 0) + (u.hp < u.def.maxHp * 0.25 ? partBonus(u, 'lastStandCore') * 500 : 0) + (u.gritUntilEndOfEnemyPhase ? 400 : 0) + (partBonus(u, 'lowHpArmor') > 0 && u.hp < u.def.maxHp * 0.4 ? 400 : 0) + (u.sanctumUntilEndOfEnemyPhase ? 400 : 0) + (u.oathUntilEndOfEnemyPhase ? 600 : 0) + (u.skills?.ironbound ?? 0) * 100 + willArmor(u) + partBonus(u, 'armor') + (u.phase2 ? 300 : 0) + (t.def > 0 ? partBonus(u, 'fortArmor') : 0);
...
export function applyAttack(state: GameState, attackerUid: string, defenderUid: string, weaponId: string, mods?: (u: UnitState) => CombatMods, reaction: Reaction = 'counter'): { state: GameState; result: AttackResult } {
  ...
  att.judgeNext = false;
  att.goreNext = false; att.rampageNext = false; att.hemoNext = false; att.voidedgeNext = false; att.plunderNext = false; att.knockNext = false; att.lacerateNext = false; att.hellfireNext = false; att.thrillNext = false; att.sunderstormNext = false; att.ravenousNext = false; att.quakeedgeNext = false; att.rageverseNext = false;
