...
function buildMission(ch: ChapterDef, pilotProg: Store['pilotProg'], upgrades: UpgradeMap, wupg: WeaponUpgMap, deploySel: string[], ngPlus: number, parts: Record<string, string[]>, difficulty: 'normal' | 'hard' | 'extreme' = 'normal', vossenAllied = false, woundedIds: string[] = []): { map: MapDef; units: UnitState[] } {
  ...
  for (const s of map.playerSpawns) {
    ...
    if (prog) {
      ...
      u.exp = prog.exp;
      u.pp = prog.pp ?? 0;
      u.skills = { hit: 0, evade: 0, dmg: 0, def: 0, countercut: 0, esave: 0, hitrun: 0, crit: 0, scavenger: 0, regen: 0, riposte: 0, lastStand: 0, assassin: 0, brawler: 0, initiative: 0, gunner: 0, plunderer: 0, ...(prog.skills ?? {}) };
...
export const useGame = create<Store>((set, get) => ({
  ...
  allocPP: (defId, statId) => {
    ...
    const skills = { hit: 0, evade: 0, dmg: 0, def: 0, countercut: 0, esave: 0, hitrun: 0, crit: 0, scavenger: 0, regen: 0, riposte: 0, lastStand: 0, assassin: 0, brawler: 0, initiative: 0, gunner: 0, plunderer: 0, ...(prog.skills ?? {}) };
