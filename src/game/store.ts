...
export const useGame = create<Store>((set, get) => ({
  ...
  captureUnit: (uid, targetUid) => {
    ...
    const salvage = Math.round((60 * t.level + (t.elite ? 200 : 0)) * (1 + 0.15 * (u.skills?.plunderer ?? 0)));
    const units = s.units.map((x) => {
    ...
    if (sp === 'cleanseverse') {
      ...
      for (const e of units) {
        if (e.alive && e.side === src.side && dist(e.pos, src.pos) <= 3) {
          ...
          e.sirenTurns = 0;
          e.rustTurns = 0;
          e.stifleTurns = 0; e.rotTurns = 0; e.frailTurns = 0; e.gloomTurns = 0; e.surgeUntilEndOfEnemyPhase = false; e.damperUntilEndOfEnemyPhase = false; e.tangleUntilEndOfEnemyPhase = false; e.leechUntilEndOfEnemyPhase = false; e.thornUntilEndOfEnemyPhase = false; e.surtaxUntilEndOfEnemyPhase = false; e.huskUntilEndOfEnemyPhase = false;
          n++;
        }
      }
