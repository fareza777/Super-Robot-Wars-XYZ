...
export const useGame = create<Store>((set, get) => ({
  ...
  castSpirit: (uid, sp) => {
    ...
    set({
      ...
      spiritForUid: null,
      usedSupport: true,
      log: trustLog || purgeLog || cheerLog || wishLog || gravityLog || decoyLog || hymnLog || empLog || phalanxLog || sanctLog || awakenLog || charityLog || siphonLog || empowerLog || marksmanLog || wardLog || warsongLog || relayLog || inspireLog || veilLog || cantataLog || warhornLog || interfereLog || safeguardLog || suppressLog || dischordLog || absolveLog || bannerLog || armorrotLog || winterverseLog || litanyLog || lockcascadeLog || wardmistLog || aegisLog || firelinkLog || dreadverseLog || sanctumLog || staticchoirLog || dirgemistLog || chorusLog || ariaLog || pinverseLog || tideverseLog || graceLog || mirrorwallLog || breachLog || ebbLog || renewalLog || valiantLog || exposeLog || darkLog || foresightLog || bastionLog || tracerLog || sirenLog || clarionLog || rampartLog || curseLog || vigorLog ? push(push(s.log, `${u.def.name} uses ${SPIRITS[sp].name}`), [trustLog, purgeLog, cheerLog, wishLog, gravityLog, decoyLog, hymnLog, empLog, phalanxLog, sanctLog, awakenLog, charityLog, siphonLog, empowerLog, marksmanLog, wardLog, warsongLog, relayLog, inspireLog, veilLog, cantataLog, warhornLog, interfereLog, safeguardLog, suppressLog, dischordLog, absolveLog, bannerLog, armorrotLog, winterverseLog, litanyLog, lockcascadeLog, wardmistLog, aegisLog, firelinkLog, dreadverseLog, sanctumLog, staticchoirLog, dirgemistLog, chorusLog, ariaLog, pinverseLog, tideverseLog, graceLog, mirrorwallLog, breachLog, ebbLog, renewalLog, valiantLog, exposeLog, darkLog, foresightLog, bastionLog, tracerLog, sirenLog, clarionLog, rampartLog, curseLog, vigorLog].filter(Boolean).join(' · ')) : push(s.log, `${u.def.name} uses ${SPIRITS[sp].name}`),
